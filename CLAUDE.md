...
**NAPRAWIONE (2026-07-23): `dim_campaign[Health OK]` (slicer) vs `Kampanie OK`/`Kampanie Problem` (karta SVG) — sprzeczne kategorie.** Slicer filtruje KOLUMNĘ obliczaną `dim_campaign[Health OK]` (liczoną raz przy odświeżeniu, z całej historii, bez filtrów). Karta liczyła przez MIARĘ `_Health Status (Emisje)` (na żywo, respektuje filtry strony). Ten sam wzór, inny kontekst → dla ~11 granicznych kampanii kolumna mówi "Problem" a miara "OK" → karta pokazywała np. "7 bez opóźnień" mimo slicera=Problem. **Fix**: `Kampanie OK`/`Kampanie Problem` przepięte na liczenie z TEJ SAMEJ kolumny co slicer: `CALCULATE(DISTINCTCOUNT(dim_campaign[campaign_id]), KEEPFILTERS(dim_campaign[Health OK]="✅ OK"/"⚠️ Problem")) + 0`. `KEEPFILTERS` → przecina się ze slicerem (nie nadpisuje), `+0` → pokazuje 0 zamiast BLANK. Gwarancja: slicer=Problem→0 OK, slicer=OK→0 problem (zweryfikowane: 0/128 i 804/0). **Świadomy tradeoff**: karta liczy teraz wg klasyfikacji whole-history (kolumna), więc NIE reaguje na filtr daty na stronie — to cena za to, że da się po tym slicować (miary nie da się). Ikony ✅/❌ w TABELI Campaign Health nadal używają miary live (grain line-item, respektują filtry) — mogą się różnić od karty (inny grain), to OK, nie bug.
...
- **`resources_backup.tar.gz`** (obok `playlog_backup.tar.gz` na Desktopie, `C:\Users\janr\Desktop\BroadsignBackup\`, 347 plików, 2025-06-27..2026-06-08) — **WGRANY 2026-07-29** przez nowy `Pipeline/bronze/import_resources_history.py` (jednorazowy, idempotentny, ADD-ONLY: dodaje tylko ID nieobecne w bronze, nigdy nie nadpisuje świeższej nazwy z codziennego fetcha — 1993 ID ma dziś inną nazwę niż w archiwum). Archiwum wgrane na VM do `/dane/BroadsignBackup/`. Wynik: `resources_latest` 8333 → **8377** (+44 ID: 34 reservation, 7 content, 2 host, 1 display_unit).
  - **Efekt na model: ZERO** (zmierzone, nie założone). `resources_latest` konsumuje wyłącznie `build_dim_content`, tylko `type=='content'`, tylko jako `fillna` dla nazw contentu **już obecnego w `ctrl_content`**. `dim_content` miał 0 braków nazw przed importem i ma 0 po; przebudowa dała 3046 → 3046 wierszy, 0 nowych/zniknionych ID, 0 zmienionych nazw. Wartość importu jest więc **ubezpieczeniowa** (słownik kompletniejszy na przyszłe joiny), nie naprawcza. Nie oczekiwać po tym poprawy czegokolwiek w PBI.
  - **Alternatywne źródło słownika**: `ScriptLinux.py` utrzymuje własną, niezależną akumulację w `/dane/OneDrive/Pulpit/UbuntuSynch/ALL_RESOURCES_LATEST.csv` (TSV `id`→`val`, odświeżany codziennie o 9:01). Po imporcie ma tylko 18 ID ponad nasz bronze (5 używanych w `fact_play_logs`) — czyli oba słowniki są praktycznie równoważne. To ten plik dostarczył 4 nazwy, których nie było w archiwum (patrz niżej).
...
- **Osobna sprawa, częściowo naprawiona**: ~170k wierszy `fact_play_logs` ma `line_item_id = NULL` już w źródle Control API (nie ID którego nie da się dociągnąć przez GET-by-ID — po prostu nigdy go nie było, `proposal_id`/`contract_id`/`line_item_id` wszystkie puste natywnie w `ctrl_reservations_v22`). To tylko **20 unikalnych `reservation_id`**, skrajnie skoncentrowane: **`1125116202` = 96,6% (164 678/170 411 wierszy)**, jeden content ("Uwaga Pociąg" — czujka na stacji przerywa pętlę komunikatem o nadjeżdżającym pociągu, czas kontraktowo należy do Metra Warszawskiego, 2025-06-26..dziś).
  ...
  - **Pozostałe 19 rezerwacji (5733 wiersze, 3,4%) — NAPRAWIONE 2026-07-22.** User zdecydował: wszystkie 19 ręcznie oznaczone `campaign_name="Autopromocja"`, `advertiser/client="Stroer"`, `line_item_name="Autopromocja"` — ściśle po tych konkretnych `reservation_id`, NIE jako ogólna reguła dla wszystkiego nieprzypisanego. Wspólny synthetic `campaign_id=900000002` (jedna kampania), unikalny `line_item_id` 900000003..900000021 per rezerwacja (inaczej `dim_line_item` dostałby duplikat klucza). `build_dim_campaign.py` dedupikuje syntetyczne wiersze po `campaign_id`.
  - **⚠️ 2026-07-29: ODNALEZIONO REALNE NAZWY wszystkich 20 — etykieta „Autopromocja" jest merytorycznie błędna.** Decyzja z 22.07 była podjęta przy BRAKU nazw. 16/20 nazw wyszło z `resources_backup.tar.gz`, pozostałe 4 z `ALL_RESOURCES_LATEST.csv` (ScriptLinux). **Żadna z 19 nie jest autopromocją Ströera** — to testy, eksperymenty synchro i widgety:
    `test_IT_synchro`, `Sychro_test_B1_B2`, `Synchro_test_b1_b2`, `synchro_matrix`, `synchro_cisza`, `synchro_kule`, `B9D_test_campaign_flight_2`, `B18D_test_campaign_flight_2`, `Programmatic test`, `Roadside_test_billboard`, `LED_Kuchnia`, `Kuchnia_LED2`, `Test na wyjazd do Zakopca`, `PKP KAT`, `PKP KAT V2`, `Amazon_licznik_test`, `Statscore_test_widget`, `Amazon Neptune - odliczanie - 22.06`, `Amazon Neptune - odliczanie - 26.06`.
    Materialność całych 20: 173 355 wierszy / 2,38 mln emisji = **0,656%** emisji `fact_play_logs`, z czego `1125116202` („Uwaga Pociąg") to 93% (167 622 wiersze / 2,21 mln emisji). Same 19 „Autopromocji" to ~0,047% emisji — marginalne.
    Sygnał potwierdzający „to testy": 15 z 19 ma **0 impresji**. Impresje mają tylko 4: `Amazon_licznik_test` (37,4 mln), `Statscore_test_widget` (11,3 mln), `Amazon Neptune 22.06` (2,76 mln), `Amazon Neptune 26.06` (1,62 mln) — i te akurat nie są autopromo, tylko licznik/odliczanie dla Amazona (treść klienta) oraz widget Statscore.
    **DECYZJA USERA 2026-07-29: ODRZUCONE, nie wracać do tematu.** Zaproponowałem podmianę na realne nazwy + przeniesienie testów z `is_serwisowy=1` na `=2`; user: „olej to, nic nie zmienia". Uzasadnienie jest po jego stronie: 0,047% emisji, a `is_serwisowy` w obu wariantach (1 i 2) i tak jest filtrowane z raportów. Etykieta „Autopromocja" ZOSTAJE mimo że merytorycznie nietrafna. Nazwy zapisane wyżej wyłącznie jako dokumentacja tego, czym te rezerwacje faktycznie są — NIE jako backlog do wdrożenia.
