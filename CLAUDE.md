...
- **Osobna sprawa, częściowo naprawiona**: ~170k wierszy `fact_play_logs` ma `line_item_id = NULL` już w źródle Control API (nie ID którego nie da się dociągnąć przez GET-by-ID — po prostu nigdy go nie było, `proposal_id`/`contract_id`/`line_item_id` wszystkie puste natywnie w `ctrl_reservations_v22`). To tylko **20 unikalnych `reservation_id`**, skrajnie skoncentrowane: **`1125116202` = 96,6% (164 678/170 411 wierszy)**, jeden content ("Uwaga Pociąg" — czujka na stacji przerywa pętlę komunikatem o nadjeżdżającym pociągu, czas kontraktowo należy do Metra Warszawskiego, 2025-06-26..dziś).
  ...
  - **⚠️ 2026-07-29: ODNALEZIONO REALNE NAZWY wszystkich 20 — etykieta „Autopromocja" jest merytorycznie błędna.** Decyzja z 22.07 była podjęta przy BRAKU nazw. 16/20 nazw wyszło z `resources_backup.tar.gz`, pozostałe 4 z `ALL_RESOURCES_LATEST.csv` (ScriptLinux). **Żadna z 19 nie jest autopromocją Ströera** — to testy, eksperymenty synchro i widgety:
    ...
    **Rekomendacja (czeka na decyzję usera, NIE wdrożona):** podmienić `campaign_name`/`line_item_name` na realne nazwy i przenieść testy z `is_serwisowy=1` (autopromo) na `is_serwisowy=2` (testowe/single-panel) — dziś zawyżają mianownik autopromo w `Fill Rate + AutoPromo`. Nazwa `1125116202` = „Uwaga Pociąg" (ręczna) jest LEPSZA niż źródłowa `TrainArrivingMessage` — tę zostawić.
